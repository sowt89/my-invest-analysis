...
사용: python3 scripts/backtest_momentum.py [상위N] [시작연도] [시장: us|kr]
"""
...
CONTROL_SEEDS = 200
MARKETS = {
    "us": {"watchlist": WATCHLIST, "bench": "QQQ",
           "eras": [("2000-2007 닷컴버블·회복", "2000", "2008"),
                    ("2008-2013 금융위기·회복", "2008", "2014"),
                    ("2014-2019 강세장", "2014", "2020"),
                    ("2020-2026 코로나 이후", "2020", "2027")]},
    "kr": {"watchlist": WATCHLIST_KR, "bench": "069500.KS",   # KODEX 200
           "eras": [("2008-2013 금융위기·회복", "2008", "2014"),
                    ("2014-2019 강세장", "2014", "2020"),
                    ("2020-2026 코로나 이후", "2020", "2027")]},
}
...
def run(px, top_n, start_year, bench):
...
def main():
    ...
    print(f"[{market.upper()}] 모멘텀 상위 {top_n}종목 · {start}년~ · 월 1회 교체\n")
    ...
    bench_label = {"QQQ": "QQQ 매수 후 보유", "069500.KS": "KODEX 200 매수 후 보유"}[bench]
    ...
    if len(q) > len(rows) * 0.9:
        g = grow(q)
        print(f"{bench_label:22s} {g:9.1f}배 {(g ** (1 / yrs) - 1) * 100:7.1f}% {mdd(q):8.1f}%")
    else:
        print(f"{bench_label:22s}   구간 일부만 존재 ({len(q)}/{len(rows)}개월) — 비교 생략")

    print("\n[시대별 균등보유 대비 배수]")
    for label, lo, hi in cfg["eras"]:
